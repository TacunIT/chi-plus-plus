...
Arte, scienza, filosofia e religione sono tutti tentativi di dare una risposta alle stesse domande sulla nostra esistenza. 
Dato che l'oggetto di studio è lo stesso, è più che normale che, alle volte, le risposte si somiglino, così come l'algoritmo per il calcolo del fattoriale di un numero o del massimo comun divisore di due cifre sarà più o meno lo stesso, indipendentemente dal linguaggio di programmazione.  
...
Ogni ricorsione è potenzialmente infinita, come l'immagine riflessa da due specchi posti l'uno di fronte all'altro.
Le funzioni ricorsive 

 vanno utilizzate con parsimonia, perché causano un aggravio di carico sul sistema.

<a href="/man/note/#ricorsione" class="nota"></a>
n questo caso, come del resto per tutte le ricorsioni in coda è generalmente preferibile utilizzare una iterazione in luogo di una ricorsione.
Consideriamo che, in linea di principio, una funzione può essere chiamata in ogni punto di un programma, è quindi necessario che venga memorizzato il suo indirizzo di ritorno, tale indirizzo viene registrato in uno stack di tipo LIFO (Last In First Out), ed insieme ad esso vengono registrate anche tutte le variabili locali. Lo stack viene liberato ad ogni uscita dalla funzione, dei dati relativi alla funzione stessa, quindi nel caso della ricorsione la liberazione dello stack avviene solamente al termine della ricorsione stessa e vengono memorizzati contemporaneamente tanti set di dati (indirizzo di ritorno e variabili locali) per quante volte viene chiamata ricorsivamente la funzione.
Dato che un qualsiasi stack di memoria ha una dimensione limitata, anche se di diverse migliaia di byte, l’esempio precedente potrebbe causare uno stack-overflow se dovessimo leggere una grande quantità di dati, pertanto, in questo caso, è preferibile una versione iterattiva della funzione.
Nella ricorsione, per evitare di superare le dimensioni dello stack, è necessario tenere presente quella che viene definita la profondità di ricorsione, oggetto del capo successivo.
