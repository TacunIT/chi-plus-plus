...
La funzione `log` è quella che abbiamo visto nella <a href="/it/funzioni.html#parametri-variabili" class="xref">lezione sulle funzioni con parametri variabili</a> e ci permette di conoscere il nome del file che viene aperto durante l'esecuzione del programma.  
...
Come certamente avrai notato, il codice di questo programma è una rielaborazione del codice della <a href="/it/stream.html" class="xref">lezione sugli stream</a>, che doveva gestire tre parametri da riga di comando.
...
> {{ site.data.citazioni.croce.testo }}<a class="nota" href="/it/note#fn-croce" id="croce">1</a>
...
Questo approccio *bottom-down*<a class="nota" href="/it/note#fn-bottom-down" id="bottom-down">2</a> ha due pregi: il primo  è che, analizzando gli eventi passati potresti scoprire che alcune colpe che ti attribuivi non erano reali; il secondo è che sarà più difficile mentire a te stesso.
...
> {{ site.data.citazioni.deen.testo }}<a class="nota" href="/it/note#fn-deen" id="deen">3</a>
...
Quello che possiamo e che dobbiamo fare, però, è di <a id="pentimento" href="/it/istruzioni-condizionali#pentimento" class="xref">pentircene sinceramente</a>, ovvero riconoscerli come errori, in modo da evitare di ripeterli nei prossimi cicli di esistenza.  
...
> {{ site.data.citazioni.mulino-amleto.stoltezza }}<a class="nota" href="/it/note#fn-stoltezza" id="stoltezza">4</a>
...
Il nostro software può variare, ma il firmware<a class="nota" href="/it/note#fn-firmware" id="firmware">5</a> che definisce il nostro comportamento a basso livello è immutato da migliaia di anni e ci spinge a fare ciò per cui siamo stati creati, ovvero riprodurci.  
